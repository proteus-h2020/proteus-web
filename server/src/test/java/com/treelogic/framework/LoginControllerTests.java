...
import com.treelogic.framework.controller.LoginController;
...
public class LoginControllerTests {
	...
	public void testGet200() throws Exception {
		mockMvc.perform(get(LoginController.PATH)).andExpect(status().is4xxClientError()); // Login
																							// has
																							// not
																							// GET
																							// method
	...
	/**
	 * @Before public void setup() { FilterProvider filter = new
	 *         SimpleFilterProvider().addFilter(ExcludeFieldsAdvice.FILTER_NAME,
	 *         SimpleBeanPropertyFilter.serializeAllExcept());
	 *         objectMapper.setFilterProvider(filter); }
	 * 
	 * @Test public void testGet200() throws Exception {
	 *       mockMvc.perform(get(MissionController.PATH)).andExpect(status().isOk());
	 *       }
	 * 
	 * @Test public void testPost400() throws Exception {
	 *       mockMvc.perform(post(MissionController.PATH)).andExpect(status().isBadRequest());
	 *       }
	 * 
	 * @Test public void testPost415() throws Exception { Mission mission = new
	 *       Mission();
	 *       mockMvc.perform(post(MissionController.PATH).content(objectMapper.writeValueAsString(mission)))
	 *       .andExpect(status().isUnsupportedMediaType()); }
	 * 
	 * @Test public void testPost400English() throws Exception {
	 *       testPost400Locale(Locale.ENGLISH); }
	 * 
	 * @Test public void testPost400Spanish() throws Exception {
	 *       testPost400Locale(Locale.forLanguageTag("es")); }
	 * 
	 * @Test public void testPost200() throws Exception { Mission mission = new
	 *       Mission(); mission.setStatus(MissionStatus.REGISTERED);
	 ...
	 *       }
	 * 
	 * @Test public void testGetAfterPost200() throws Exception {
	 *       mockMvc.perform(get(MissionController.PATH)).andExpect(status().isOk())
	 ...
	 *       HttpError httpError = objectMapper.readValue(content,
	 *       HttpError.class); httpError.getMessage()
	 *       .equals(messageSource.getMessage("NotNull", new String[] { null,
	 *       "keycloakUser", null }, locale)); }
	 **/
}